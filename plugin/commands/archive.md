...
5. **Delete:** `git rm` every manifest row's `path` with `disposition: archive`; `git rm` every `reviews:` entry with `action: delete`; `git mv` every `action: move-to-archive` review to `docs/archive/`; `git rm` the manifest itself (recoverable at the tag). For an EXPL row, remove the WHOLE exploration directory — `git rm -r docs/explorations/EXPL-<id>-<slug>/` — never just the inventoried `map.md` file.
