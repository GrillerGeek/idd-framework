...
Terminal Expectations (`done`/`validated`, or `deferred` with no live obligation) are eligible for `the archive workflow` — distilled into `docs/idd-ledger.yaml` and removed from disk (full text recoverable via the archive git tag). There is no `archived` status.
