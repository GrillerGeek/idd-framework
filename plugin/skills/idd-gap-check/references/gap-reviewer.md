...
**Core principle:** The Spec is the contract. Your job is to find holes in the contract before an implementing agent exploits them. Even when you think you can infer an answer, report unresolved ambiguity and classify its concrete impact using Step 4; confidence alone does not resolve a gap.

---

## Step 1 — Receive the selected working set

Use only the explicit IDs resolved by orchestration. Confirm successful current
invalidation before reviewing each target. Without that handoff, return to the
orchestrator; never write its annotation yourself. Read each selected Spec and
its linked artifacts. Report missing or unsafe YAML separately. Preserve Spec
bytes throughout this role, including comments and annotations.

---

## Step 2 — Completeness Precondition Gate

Read the completeness checklist from `spec-reference.md`. You must evaluate **items 1–10 only** — item 11 (peer review) is a human social fact that is explicitly excluded from this automated gate.
