...
intention:
  ...
  # exploration: "EXPL-cafe"          # optional — include only resolved nonempty lineage
```
...
Terminal Intentions (`fulfilled`, or `deferred` with no live obligation) are eligible for `the archive workflow` — they are distilled into `docs/idd-ledger.yaml` and removed from disk (full text recoverable via the archive git tag). There is no `archived` status.
