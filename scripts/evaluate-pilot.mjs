...
export async function evaluatePilot({host,scenario,output,'claude-output-style':outputStyle='configured'}) {
  ...
  if(host==='claude')result.outputStyle=outputStyle;
  try {
...
if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)) {
  try {
    ...
    for(let i=0;i<args.length;i+=2){if(!['--host','--scenario','--output','--claude-output-style'].includes(args[i])||!args[i+1])throw Error('Expected --host, --scenario and optional --output / --claude-output-style pairs');options[args[i].slice(2)]=args[i+1];}
