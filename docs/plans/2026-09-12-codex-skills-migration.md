...
- Focused Claude checkpoint recovery adds an opt-in staged evaluation controller: two read-only acknowledgments in the same saved session, then implementation and controller-owned final verification/status. A real configured-model/ELI5 run passed independent trace review, with 101 offline tests and all sixteen installer combinations. This is development tooling, not yet a bundled/native execution path; ordinary Claude and the broader pilot gate remain open. See the [checkpoint recovery report](../reviews/2026-09-12-claude-checkpoint-recovery.md) for scope and evidence qualifications.
