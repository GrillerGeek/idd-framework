...
Reviewed Spec SHA256: `40ce86fff457df05b61009509d23086866fc864f4bcb9bbfc43e8efeaac19633`
...
The sweep used Deliverable path basenames (including directory basenames such as `references`, `helpers`, and `lib`) and introduced/changed terms such as `MENTIONS`; matched whole tokens were used for generic words. It searched tracked repository text, including dotfiles, templates and examples, excluding the five artifact/review trees. Declared directory allowlists own their contained files. Broad directory-basename matches are retained as candidates even where their meaning is incidental. All 33 candidates below retain their original Warning severity and are now resolved. The author recorded six reasoned accept-omission groups. Independent review confirms the linked contributor-guide ownership, unchanged helper interfaces and installed resource paths, unchanged template schema, and protected historical context; none has an unmet validation dependency. The added development-code SPDX convention is implementable within owned scripts/tests and contributor documentation without relicensing existing material or requiring an unowned LICENSE edit. Resolved candidates contribute zero to gate counts. The initial unresolved review remains in `docs/reviews/SPEC-bffd-gap-check-initial.md`.
...
- **file:** `.cursor/rules/idd.mdc`
  ...
  **resolution:** resolved — Independently confirmed: This entry point already delegates source ownership and runnable checks to the owned shared contributor guide; helper names and the linked conventions anchor remain stable.
...
- **file:** `docs/artifacts.md`
  ...
  **resolution:** resolved — Independently confirmed: The matched term describes existing IDD semantics rather than the new assembler API; the five-block schema, lifecycle and semantic review process remain unchanged.
...
- **file:** `docs/plans/2026-09-12-codex-skills-migration.md`
  ...
  **resolution:** resolved — Independently confirmed: The migration plan is author/orchestrator tracking outside this implementation allowlist; its milestone status can be maintained separately without affecting foundation validation.
...
- **file:** `docs/superpowers/plans/2026-07-28-exploration-phase-0.md`
  ...
  **resolution:** resolved — Independently confirmed: This is historical or worked-example context, protected by the Boundary; the new tooling does not change the example behavior or require historical text updates.
...
- **file:** `plugin/agents/deep-review-lead.md`
  ...
  **resolution:** resolved — Independently confirmed: The existing Claude consumer keeps the same installed resource/helper paths and interfaces; command/agent edits are expressly excluded by the compatibility Boundary.
