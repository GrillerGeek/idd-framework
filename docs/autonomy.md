...
AI agents make this problem worse, not better. An AI agent that encounters ambiguity doesn't wait — it guesses. And it guesses at machine speed, producing wrong output faster than anyone can catch it. IDD addresses this through a pre-execution adversarial gap-check. Execution requires a ready Spec and a current passed result with zero unresolved Blockers or Warnings. Authors fix substantive findings in the Spec; accepted coverage omissions remain visible and only become resolved after independent confirmation. During execution, a gap that changes validation outcomes, risks crossing a Boundary, or changes a Deliverable's shape requires stopping and reporting. Minor choices within that contract can proceed with documented reasoning. These are agent and human protocol obligations, not an executable machine guarantee that all ambiguity has been eliminated. See the [Spec lifecycle contract](framework.md#spec-lifecycle-contract).
