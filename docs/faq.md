...
The completeness checklist verifies that every Spec block is *present* — it cannot tell whether the content is any good. The gap-check gate closes that hole: before execution, a reviewing agent simulates being the implementer and reports every point where it would have to guess — ambiguous Expectations, contradictions between blocks, filler edge cases, unverifiable validation criteria. Findings are classified as Blockers (implementations would diverge wrongly) or Warnings (implementations would be inconsistent), and execution requires ready plus a current passed gap-check with zero unresolved Blockers and Warnings. All eleven readiness items include actual human peer review; AI review cannot supply it. Crucially, the gap-checker never fixes anything: gaps are resolved in the Spec by its author, so the Spec remains the complete contract. In practice, the gate's biggest catches are confident factual errors — a Spec describing a codebase reality that doesn't exist — which no presence-based checklist can see. The gate also performs a coverage sweep — reporting files that reference the changed concepts but are owned by no Deliverable — because the most expensive gaps are often omissions, not defects: a published document that references a concept you renamed but was never assigned to any Spec will silently diverge from the release.

An author-accepted coverage omission remains visible as resolved only after the
reviewer confirms its reason leaves no unmet validation dependency. Substantive
content warnings cannot be waived by acknowledgment. Failed or interrupted
checks invalidate old success; a prior report alone never authorizes a build.
See the [lifecycle contract](framework.md#spec-lifecycle-contract).
